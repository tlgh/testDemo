...
public class JpaQuery implements Serializable {
	...
	private JpaQuery(Class entityClass, EntityManager entityManager) {
		this.entityClass = entityClass;
		this.entityManager = entityManager;
		this.criteriaBuilder = this.entityManager.getCriteriaBuilder();
		this.criteriaQuery = criteriaBuilder.createQuery(this.entityClass);
		this.from = criteriaQuery.from(this.entityClass);
	...
	public void or(List<String> propertyName, Object value) {
		...
		if ((propertyName == null) || (propertyName.size() == 0))
			return;
		Predicate predicate = criteriaBuilder.or(criteriaBuilder.equal(
				from.get(propertyName.get(0)), value));
		for (int i = 1; i < propertyName.size(); ++i)
			predicate = criteriaBuilder
					.or(predicate, criteriaBuilder.equal(
							from.get(propertyName.get(i)), value));
	...
	public void orLike(List<String> propertyName, String value) {
		...
		if (value.indexOf("%") < 0)
			value = "%" + value + "%";
		Predicate predicate = criteriaBuilder.or(criteriaBuilder.like(
				from.get(propertyName.get(0)), value.toString()));
		for (int i = 1; i < propertyName.size(); ++i)
	...
	public void notEq(String propertyName, Object value) {
		...
		this.predicates.add(criteriaBuilder.notEqual(from.get(propertyName),
				value));
	}
	...
	public void like(String propertyName, String value) {
		...
		if (value.indexOf("%") < 0)
			value = "%" + value + "%";
		this.predicates
				.add(criteriaBuilder.like(from.get(propertyName), value));
	}
	...
	public void between(String propertyName, Date lo, Date go) {
		if (!isNullOrEmpty(lo) && !isNullOrEmpty(go)) {
			this.predicates.add(criteriaBuilder.between(from.get(propertyName),
					lo, go));
		}
	...
	private void addLinkCondition(JpaQuery query) {
		...
		for (Iterator queryIterator = subQuery.keySet().iterator(); queryIterator
				.hasNext();) {
