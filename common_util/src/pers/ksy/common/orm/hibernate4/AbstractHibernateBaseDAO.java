...
public abstract class AbstractHibernateBaseDAO<T, ID extends Serializable> extends
		HibernateSimpleDAO implements HibernateBaseDAO<T, ID> {
	...
	public T getByProperty(String propName, Object propVal, String[] joins) {
		DetachedCriteria dc = buildFindByPropertyDC(new String[] { propName },
				new Object[] { propVal }, joins);
		return (T) uniqueByDC(dc);
	}
	
	protected DetachedCriteria buildFindByPropertyDC(String[] propNames,
			Object[] propVals, String[] joins) {
		DetachedCriteria dc = getDC();
		if (null != propNames && null != propVals
				&& propNames.length == propVals.length) {
